...
public abstract class Unit : MonoBehaviour {
	...
	public int DamageReduction{
		get {
			int reduction;
			if (isDefending)
				reduction = _damageReduction + DefenseBonus;
			else
				reduction = _damageReduction;
			return reduction;
		}
		set {
	...
	//if the unit is defending
	public bool isDefending;
	//the amount of additional damage reduced by defending
	...
	public int experience{
		...
		set {
			_experience = value;
			if (_experience == 1 && level == 1)
				LevelUp ();
			else if (_experience == 3 && level == 2)
				LevelUp ();
		}
	}
	...
	//when the unit undoes a defense selection
	public UnitEvent OnDefenseUndo;
	...
	protected string runAnimName = "run";
	protected string attackAnimName = "attack";
	protected string spec1AnimName = "spec1";
	protected string spec2AnimName = "spec2";
	protected string defenseAnimName = "defense";
	protected string recoilAnimName = "recoil";
	protected string deathAnimName = "death";

	protected Animator animator;
	
	public bool playAnimation(string type)
	{
		animator = GetComponent<Animator>();
		Debug.LogWarning ("playAnimation was called for " + type);
	...
	protected virtual void Awake () {
		...
		OnDefense = UnitResolveDefense;
		OnDefenseUndo = UndoDefenseSelected;
		animator = GetComponent<Animator>();
	}
	...
	//When the unit is set to defend, this is called.
	public virtual void UnitHasSelectedDefend(){
		//Deselect to remove all other ranges in play.
		OnActionDeselect ();
		//Add a move action into the last slot of the action stack.
		TurnActionOrderHandler.Instance.actionList.AddLast (new Defense (Position, layer));
		HasInteracted = true;
		try {
			animator.SetBool ("IsDefending", true);
		}
		catch {
			Debug.LogWarning (MyName + " does not have a defend animation.");
		}
		GameManager.Instance.OnTurnBegin += DefenseCounter;
	}

	public virtual void UndoDefenseSelected(){
		isDefending = false;
		HasInteracted = false;
		OnActionSelect ();
		try {
	...
	public virtual void UnitResolveDefense (){
		HasInteracted = false;
	}

	public virtual void DefenseCounter () {
	...
	public virtual void UnitHasSelectedAttack(Vector2 TargetPosition, Vector2 InitiatorPosition, int TargetLayer, int InitiatorLayer){
		...
		try {
			...
			Debug.LogWarning (MyName + " does not have an attack animation.");
		}
	}
		...
		if (TargetUnit != null) {
			...
			if (!TargetUnit.IsKing || (TargetUnit.IsKing && TargetUnit.UnitOwner.WallIsDestroyed)){
				...
				for (int rolls = 0; rolls < level; rolls++){
					...
					if (randomNumber <= TargetUnit.DefenseChance){
	...
	public virtual void spec1Anim (Vector2 TargetPosition, Vector2 InitiatorPosition, int TargetLayer, int InitiatorLayer) {
		animator.SetTrigger ("spec1");
	}

	public virtual void spec2Anim (Vector2 TargetPosition, Vector2 InitiatorPosition, int TargetLayer, int InitiatorLayer) {
		animator.SetTrigger ("spec2");
	}
	...
	public virtual void InsertGUI () {
		...
		UnitInfo_Health.Instance.Label.text = Health + "/" + MaxHealth;
